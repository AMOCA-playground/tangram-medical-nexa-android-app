...
/**
 * Result types for LLM generation.
 */
sealed class LlmGenerationResult {
    data class Token(val text: String) : LlmGenerationResult()
    data object Completed : LlmGenerationResult()
    data class Error(val throwable: Throwable) : LlmGenerationResult()
}
...
class NexaLlmEngine private constructor(
     ...
     * Explains medical terms or provides drug information.
     */
    fun chatWithMedicalAssistant(
        ...
        withContext(Dispatchers.IO) {
            ...
            val chatMessages = arrayListOf(
                ChatMessage("system", MEDICAL_ASSISTANT_PROMPT)
            )
            ...
            templateResult.onSuccess { template ->
                ...
                wrapper.generateStreamFlow(template.formattedText, genConfig).collect { result ->
                    when (result) {
                        is LlmStreamResult.Token -> {
                            send(LlmGenerationResult.Token(result.text))
                        }
                        is LlmStreamResult.Completed -> {
                            wrapper.reset()
                            send(LlmGenerationResult.Completed)
                        }
                        is LlmStreamResult.Error -> {
                            Log.e(TAG, "Medical assistant chat error", result.throwable)
                            send(LlmGenerationResult.Error(result.throwable))
                        }
                    }
                }
            }.onFailure { error ->
                Log.e(TAG, "Failed to apply chat template", error)
                send(LlmGenerationResult.Error(error))
            }
        }
    }
...
Return only the SOAP note. No explanations, no preamble, no post-notes."""
         ...
         */
        const val MEDICAL_ASSISTANT_PROMPT = """You are a medical assistant specialized in explaining medical terms and providing drug information. 
Your goal is to provide clear, concise, and accurate information based on medical knowledge.

Rules:
1. Explain medical terms in simple language that a patient can understand, but remain clinically accurate.
2. For drug information, provide: 
   - Generic name and common brand names
   - Primary uses (indications)
   - Common side effects
   - Important warnings or contraindications
3. Always include a disclaimer: "This information is for educational purposes only and does not substitute for professional medical advice, diagnosis, or treatment. Always seek the advice of your physician or other qualified health provider."
4. If you don't know the answer or the term is not medical, politely state that you can only help with medical terms and drug info.
5. Be professional and empathetic.

Return your response in a structured and easy-to-read format."""
    }
}
