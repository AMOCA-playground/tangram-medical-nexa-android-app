...
interface RecordingNoteDao {
    ...
    @Query("SELECT COUNT(*) FROM recording_notes")
    suspend fun count(): Int

    @Query("SELECT COUNT(*) FROM recording_notes WHERE status = :status")
    ...
    @Query("""
        ...
        WHERE (:status IS NULL OR status = :status)
          AND (:query IS NULL 
               ...
               OR patientId LIKE '%' || :query || '%'
               OR department LIKE '%' || :query || '%'
               OR clinicianName LIKE '%' || :query || '%')
        ORDER BY createdAtEpochMs DESC
    """)
    fun search(status: String?, query: String?): Flow<List<RecordingNoteEntity>>
}
